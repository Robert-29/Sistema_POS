...
BEGIN
  ...
  IF p_vendedor_id IS NOT NULL THEN
     SELECT rol, permisos INTO v_user_role, v_permisos FROM usuarios_sucursal WHERE id = p_vendedor_id;
     -- If user found, check permission
     IF v_user_role IS NOT NULL THEN
        IF v_user_role NOT IN ('admin') AND (v_permisos->>'can_sell')::boolean IS DISTINCT FROM true THEN
           -- RAISE EXCEPTION 'No tienes permiso para realizar ventas.';
           -- Temporarily allowing backwards compatibility or soft fail
        END IF;
     END IF;
  END IF;
